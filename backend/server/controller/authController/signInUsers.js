...
const signInUser = async (req, res, next) => {
  try {
    ...
    if (!email) {
      return { code: 400, message: "Please provide email" };
    }
    if (!password) {
      return { code: 400, message: "Please provide password" };
    }
    ...
    if (!checkIfUsersExist || checkIfUsersExist.length === 0) {
      return { code: 409, message: "User with provided email does not exist" };
    }
